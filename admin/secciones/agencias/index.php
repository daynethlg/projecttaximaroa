...
if (isset($_GET['txtid'])) {
    ...
    //imagen
    $sentencia2 = $conexion->prepare("SELECT imagen FROM tb_agencias WHERE id=:id ");
    $sentencia2->bindParam(":id", $txtid);
    $sentencia2->execute();
    $registro_imagen2 = $sentencia2->fetch(PDO::FETCH_LAZY);

    if (isset($registro_imagen2["imagen"])) {
        if (file_exists("../../../img/agencias/" . $registro_imagen2["imagen"])) {
            echo "Imagen enconconyrada...";
            unlink("../../../img/agencias/" . $registro_imagen2["imagen"]);
        }
    }


    //borra registror con ID correspondiente
    $sentencia2 = $conexion->prepare("DELETE FROM tb_agencias WHERE id=:id ");
    $sentencia2->bindParam(":id", $txtid);
    $sentencia2->execute();
}
...
$sentencia2 = $conexion->prepare("SELECT *FROM `tb_agencias`");
$sentencia2->execute();
...
if (isset($_GET['txtid'])) {
    ...
    //imagen
    $sentencia = $conexion->prepare("SELECT imagen FROM tb_agencias_categoria WHERE id=:id ");
    $sentencia->bindParam(":id", $txtid);
    $sentencia->execute();
    $registro_imagen = $sentencia->fetch(PDO::FETCH_LAZY);

    if (isset($registro_imagen["imagen"])) {
        if (file_exists("../../../img/agencias/" . $registro_imagen["imagen"])) {
            echo "Imagen enconconyrada...";
            unlink("../../../img/agencias/" . $registro_imagen["imagen"]);
        }
    }


    //borra registror con ID correspondiente
    $sentencia = $conexion->prepare("DELETE FROM tb_agencias_categoria WHERE id=:id ");
    $sentencia->bindParam(":id", $txtid);
    $sentencia->execute();
}
...
//seleccionar registros
$sentencia = $conexion->prepare("SELECT *FROM `tb_agencias_categoria`");
$sentencia->execute();
                    ...
                    <tbody>
                        <?php foreach ($lista_comida2 as $registros2) { ?>
                            ...
                            <tr class="">
                                ...
                                <td scope="col">
                                    <img width="50" src="../../../img/agencias/<?php echo $registros2['imagen']; ?>" />
                                </td>
                    ...
                    <tbody>
                        <?php foreach ($lista_comida as $registros) { ?>
                            ...
                            <tr class="">
                                ...
                                <td scope="col">
                                    <img width="50" src="../../../img/agencias/<?php echo $registros['imagen']; ?>" />
                                </td>
