...
if (isset($_GET['txtid'])) {
    ...
    $sentencia = $conexion->prepare("SELECT * FROM tb_conecta_perfil WHERE id=:id");
...
if($_POST) {
    print_r($_POST);
     ...
     $sentencia = $conexion->prepare("UPDATE tb_conecta_perfil 
     SET 
     titulo=:titulo
     WHERE id=:id ");
    ...
    if ($_FILES["imagen"]["tmp_name"] != "") {
       ...
       //boorado arcjivpo anterior
        $sentencia = $conexion->prepare("SELECT imagen FROM tb_conecta_perfil WHERE id=:id ");
        $sentencia->bindParam(":id", $txtid);
        $sentencia->execute();
