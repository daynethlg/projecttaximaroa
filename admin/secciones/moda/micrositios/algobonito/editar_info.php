...
if (isset($_GET['txtid'])) {
    ...
    $sentencia = $conexion->prepare("SELECT * FROM tb_algobonito_info WHERE id=:id");
...
if($_POST) {
    print_r($_POST);
     ...
     $sentencia = $conexion->prepare("UPDATE tb_algobonito_info 
     SET 
     titulo=:titulo,
    ...
    if ($_FILES["imagen"]["tmp_name"] != "") {
       ...
       //boorado arcjivpo anterior
        $sentencia = $conexion->prepare("SELECT imagen FROM tb_algobonito_info WHERE id=:id ");
        $sentencia->bindParam(":id", $txtid);
        $sentencia->execute();
