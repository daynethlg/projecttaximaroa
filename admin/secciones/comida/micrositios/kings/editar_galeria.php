...
if (isset($_GET['txtid'])) {
    ...
    $sentencia = $conexion->prepare("SELECT * FROM tb_kings_galeria WHERE id=:id");
...
if($_POST) {
    print_r($_POST);
     ...
     $sentencia = $conexion->prepare("UPDATE tb_kings_galeria 
     SET 
     titulo=:titulo
    ...
    if ($_FILES["imagen"]["tmp_name"] != "") {
       ...
       //boorado arcjivpo anterior
        $sentencia = $conexion->prepare("SELECT imagen FROM tb_kings_galeria WHERE id=:id ");
        $sentencia->bindParam(":id", $txtid);
        $sentencia->execute();
