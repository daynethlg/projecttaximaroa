...
if (isset($_GET['txtid'])) {
    ...
    $sentencia = $conexion->prepare("SELECT * FROM tb_kings_icono WHERE id=:id");
...
if($_POST) {
    print_r($_POST);
     ...
     $sentencia = $conexion->prepare("UPDATE tb_kings_icono 
     SET 
     titulo=:titulo,
     link=:link
     WHERE id=:id ");
    ...
    if ($_FILES["imagen"]["tmp_name"] != "") {
       ...
       //boorado arcjivpo anterior
        $sentencia = $conexion->prepare("SELECT imagen FROM tb_kings_icono WHERE id=:id ");
        $sentencia->bindParam(":id", $txtid);
        $sentencia->execute();
